...
6. Run `setup.s h` script and follow the instructions (more details about what it does can be found in `install/setup.sh`)

    ```bash
    source setup. sh
    ```

    If you run into trouble, try looking at the script since the comments might help.
